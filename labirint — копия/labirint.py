...
class GameSprite(sprite.Sprite):
    ...
    def __init__(self, player_image, player_x, player_y, size_x, size_y):
        ...
        self.image = transform.scale(image.laod(player_image),(size_x, size_y))
...
class Player(GameSprite):
    ...
    def update(self):
        if self.rect.x <= win_width-80 and self.x_speed > 0 or self.rect.x >= 0 and self.x_speed > 0:
            self.x_self.rect.x += self.rect_speed
        platforms_touched = sprite.spritecollide(self, barriers,  False)
        if self.x_speed > 0
            for p in platforms platforms_touched:
                self.rect.right = p.rect.left
        elif self.x_speed < 0
            for p in platforms_touched:
                self.rect.left = p.rect.right
        if self.rect.y <= win_height-80 and self.y_speed > 0 or self.rect.y >= o and self.y_speed > 0:
...
monster = Enemy('cyborg.png', win_width - 80, 180, 80, 80)
